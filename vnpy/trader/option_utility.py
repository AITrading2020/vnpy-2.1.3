...
class ArrayManager(object):
    ...
    def __init__(self, size: int = 100, columns=None):
        """Constructor"""
        self.count: int = 0
        self.size: int = size
        ...
        if self.columns:
            for k in self.columns:
                expr = "self." + k + "_array=np.zeros(size)"
                exec(expr)
        if self.columns:
            for k in self.columns:
                expr = "self.{}=self.{}_array".format(k, k)
                exec(expr)
    ...
    def update_bar(self, bar: BarData) -> None:
        ...
        if self.columns:
            for i in self.columns:
                expr = "self." + i + "_array[:-1] = self." + i + "_array[1:]"
                exec(expr)
                expr = "self." + i + "_array[-1] = bar." + i
                exec(expr)
